...
import SiderBondClawEntry from './SiderBondClawEntry';
...
const Sider: React.FC<SiderProps> = ({ onSessionClick, collapsed = false }) => {
  ...
  const handleBondClawClick = () => {
    cleanupSiderTooltips();
    blurActiveElement();
    closePreview();
    setIsBatchMode(false);
    Promise.resolve(navigate('/bondclaw')).catch((error) => {
      console.error('Navigation failed:', error);
    });
    if (onSessionClick) {
      onSessionClick();
      ...
      <div className='flex-1 min-h-0 overflow-hidden'>
        {isSettings ? (
          ...
          <div className='size-full flex flex-col gap-2px'>
            ...
            <SiderBondClawEntry
              isMobile={isMobile}
              isActive={pathname === '/bondclaw'}
              collapsed={collapsed}
              siderTooltipProps={siderTooltipProps}
              onClick={handleBondClawClick}
            />
