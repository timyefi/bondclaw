...
with connection.cursor() as cursor:
    ...
    # 计算近1月年化收益率和近1年年化收益率
    df['近1月年化收益率'] = df.groupby('trade_code')['NAV_ACC'].apply(lambda x: (x / x.shift(30) - 1) * 365 / 30).reset_index(level=0, drop=True)
    df['近1年收益率'] = df.groupby('trade_code')['NAV_ACC'].apply(lambda x: (x / x.shift(365) - 1)).reset_index(level=0, drop=True)
    # 计算每个日期的平均收益率
