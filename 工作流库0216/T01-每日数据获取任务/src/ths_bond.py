...
bgn_date_d = '2021-10-01'
end_date_d = '2030-10-01'
...
_df = pd.read_sql("""SELECT
            ...
            max( dt ) AS dt 
        FROM
            bond.marketinfo_curve# where trade_code = 'L001618528'
        GROUP BY
            trade_code""", conns)
for i, row in _df.iterrows():
    ...
    if not data.data.empty:
        df = pd.DataFrame()
        df = data.data[['id', 'time', 'value']].rename(columns={'id': 'TRADE_CODE', 'time': 'DT', 'value': 'CLOSE'})
        insert_database(df.fillna(''), 'bond.marketinfo_curve', ['DT', 'TRADE_CODE'])