  ...
  try {
    ...
    if (savedModel?.id && savedModel?.useModel) {
      if (savedModel.id === GOOGLE_AUTH_PROVIDER_ID) {
        ...
        if (hasLocalCreds) {
          // The google-auth-gemini provider is a frontend-only synthetic provider 鈥?it is NOT
        ...
        // No local OAuth credentials 鈥?try to fall back to an API key provider for the same model.
        console.warn(
    ...
    try {
      ...
      // credentials file missing or unreadable 鈥?fall through
    }
  } catch (error) {
    console.warn('[SystemActions] Failed to get saved model, using default:', error);
  }

  // Default fallback - minimal config for Gemini (no API key 鈥?will fail with clear error)
...
export const handleSessionStatus: ActionHandler = async (context) => {
  ...
  return createSuccessResponse({
    type: 'text',
    text: [
      '馃搳 <b>Session Status</b>',
      '',
      `馃 Agent: <code>${session.agentType}</code>`,
      `鈴?Duration: ${duration} min`,
      `馃摑 Last activity: ${lastActivity} sec ago`,
      `馃敄 Session ID: <code>${session.id.slice(-8)}</code>`,
    ].join('\n'),
    parseMode: 'HTML',
    replyMarkup: createSessionControlKeyboard(),
  });
...
export const handleHelpShow: ActionHandler = async (context) => {
  ...
  return createSuccessResponse({
    type: 'text',
    text: [
      '鉂?<b>BondClaw Assistant</b>',
      '',
      'A remote assistant to interact with BondClaw via Telegram.',
      '',
      '<b>Common Actions:</b>',
      '鈥?馃啎 New Chat - Start a new session',
      '鈥?馃搳 Status - View current session status',
      '鈥?鉂?Help - Show this help message',
      '',
...
export const handleHelpFeatures: ActionHandler = async (context) => {
  ...
  return createSuccessResponse({
    ...
    text: [
      ...
      '',
      '<b>AI Chat</b>',
      '鈥?Natural language conversation',
      '鈥?Streaming output, real-time display',
      '鈥?Context memory support',
      '',
      '<b>Session Management</b>',
      '鈥?Single session mode',
      '鈥?Clear context anytime',
      '鈥?View session status',
      '',
      '<b>Message Actions</b>',
      '鈥?Copy reply content',
      '鈥?Regenerate reply',
      '鈥?Continue conversation',
    ].join('\n'),
    parseMode: 'HTML',
    replyMarkup: createHelpKeyboard(),
  });
...
export const handleHelpPairing: ActionHandler = async (context) => {
  ...
  return createSuccessResponse({
    type: 'text',
    text: [
      '馃敆 <b>Pairing Guide</b>',
      '',
      '<b>First-time Setup:</b>',
      '1. Send any message to the bot',
      '2. Bot displays pairing code',
      '3. Approve pairing in BondClaw settings',
      '4. Ready to use after pairing',
      '',
      '<b>Notes:</b>',
      '鈥?Pairing code valid for 10 minutes',
      '鈥?BondClaw app must be running',
      '鈥?One Telegram account can only pair once',
    ].join('\n'),
    parseMode: 'HTML',
    replyMarkup: createHelpKeyboard(),
  });
...
export const handleHelpTips: ActionHandler = async (context) => {
  ...
  return createSuccessResponse({
    ...
    text: [
      ...
      '鈥?Be clear and specific',
      '鈥?Feel free to ask follow-ups',
      '鈥?Regenerate if not satisfied',
      '',
      '<b>Quick Actions:</b>',
      '鈥?Use bottom buttons for quick access',
      '鈥?Tap message buttons for actions',
      '鈥?New chat clears history context',
    ].join('\n'),
    parseMode: 'HTML',
    replyMarkup: createHelpKeyboard(),
  });
...
export const handleSettingsShow: ActionHandler = async (context) => {
  ...
  return createSuccessResponse({
    ...
    text: [
      '鈿欙笍 <b>Settings</b>',
      '',
      'Channel settings need to be configured in the BondClaw app.',
      '',
      'Open BondClaw 鈫?WebUI 鈫?Channels',
    ].join('\n'),
    parseMode: 'HTML',
...
export const handleAgentSelect: ActionHandler = async (context, params) => {
  ...
  if (existingSession?.agentType === newAgentType) {
    ...
    return createSuccessResponse({
      type: 'text',
      text: `鉁?Already using <b>${getAgentDisplayName(newAgentType)}</b>`,
      parseMode: 'HTML',
      replyMarkup: markup,
    });
  }
  ...
  return createSuccessResponse({
    type: 'text',
    text: [
      `鉁?<b>Switched to ${getAgentDisplayName(newAgentType)}</b>`,
      '',
...
function getAvailableChannelAgents(): AgentDisplayInfo[] {
  ...
  availableAgents.push({ type: 'gemini', emoji: '馃', name: 'Gemini' });
