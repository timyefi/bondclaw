...
const mapStatusToState = (
  ...
  if (lastStatus === 'error' || recentEvents.some((e) => /error|澶辫触|寮傚父/i.test(e.text))) return 'error';

  const hasWriteEvent = recentEvents.some((e) => /write|patch|edit|鍐欏叆|淇敼|鐢熸垚鏂囦欢/i.test(e.text));
  const hasResearchEvent = recentEvents.some((e) => /search|web|fetch|crawl|璋冪爺|妫€绱鎼滅储/i.test(e.text));
    ...
    for (const conversation of conversations) {
      ...
      if (!existing) {
        byAgent.set(key, {
          ...
          currentTask: events[0]?.text || (runtimeStatus === 'running' ? 'Executing' : 'Idle'),
          recentEvents: events,
        });
        continue;
      }
