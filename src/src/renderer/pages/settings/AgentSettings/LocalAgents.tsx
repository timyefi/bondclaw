...
const LocalAgents: React.FC = () => {
  ...
  // Claude first, then Aion CLI, then Gemini, then other detected agents
  const claudeAgent = detectedAgents?.find((a) => a.backend === 'claude');
  const aionrsAgent = detectedAgents?.find((a) => a.backend === 'aionrs');
  const geminiAgent = detectedAgents?.find((a) => a.backend === 'gemini');
  const otherDetected =
    detectedAgents?.filter((a) => a.backend !== 'claude' && a.backend !== 'aionrs' && a.backend !== 'gemini') ?? [];
      ...
      <div className='grid grid-cols-2 gap-10px px-16px md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5'>
        {claudeAgent && (
          <AgentCard
            ...
            onSettings={() => navigate('/settings/claude')}
            variant='grid'
          />
        )}
        {aionrsAgent && (
