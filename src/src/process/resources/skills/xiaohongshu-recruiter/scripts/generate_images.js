...
function getFontDir() {
  const candidates = [
    ...
    path.join(process.env.HOME || '', 'Library/Application Support/BondClaw/config/skills/canvas-design/canvas-fonts'),
    path.join(process.env.APPDATA || '', 'BondClaw/config/skills/canvas-design/canvas-fonts'),
  ].filter(Boolean);
...
function generateCover(title1, title2, slogan1, slogan2) {
  ...
  ctx.fillText(slogan1 || '寻找未来的定义�?, PADDING, HEIGHT - 200);
