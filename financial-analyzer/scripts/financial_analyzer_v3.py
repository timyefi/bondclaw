...
def build_chapter_record(index, chapter, report_context):
    text = "\n".join(chapter["content_lines"]).strip()
    cleaned_text = normalize_text(text)
    topic_tags = infer_topics(chapter["title"], cleaned_text)
    numeric_data = extract_numeric_data(cleaned_text)
    anomalies = build_anomalies(cleaned_text)
    findings = build_findings(chapter["title"], cleaned_text, topic_tags, numeric_data)
    evidence = build_evidence(chapter["title"], cleaned_text, numeric_data, anomalies)

    chapter_type = "notes_chapter" if "附注" in chapter["title"] or re.search(r"^\(?[0-9]{1,2}\)?", chapter["title"]) else "section"
    ...
    return {
        ...
        "attributes": {
            ...
            "line_count": len(chapter["content_lines"]),
        ...
        "evidence": evidence,
        "extensions": {
            "raw_title_tokens": extract_title_tokens(chapter["title"]),
...
def build_final_data(report_context, chapter_records, focus_list):
    ...
    conclusions.append(
        f"逐章记录共生成 {len(chapter_records)} 条，覆盖章节 {chapter_records[0]['chapter_no']} 至 {chapter_records[-1]['chapter_no']}。"
    )
...
def build_report_markdown(report_context, focus_list, final_data, pending_updates, chapter_records):
    ...
    for record in chapter_records[:12]:
        lines.append(
            f"- 第{record['chapter_no']}章 `{record['chapter_title']}`：{record['summary']}"
        )
...
def resolve_output_paths(md_path, output_arg, run_dir_arg):
    explicit_output = Path(output_arg).resolve() if output_arg else None
    run_dir = Path(run_dir_arg).resolve() if run_dir_arg else None

    if run_dir is None and explicit_output is not None:
        if explicit_output.suffix.lower() == ".xlsx":
            run_dir = explicit_output.parent / f"{explicit_output.stem}_artifacts"
        else:
            run_dir = explicit_output

    if run_dir is None:
        run_dir = md_path.parent / f"{md_path.stem}_analysis"

    run_dir.mkdir(parents=True, exist_ok=True)
    stable_excel = run_dir / "financial_output.xlsx"

    compatibility_excel = None
    if explicit_output is not None and explicit_output.suffix.lower() == ".xlsx":
        compatibility_excel = explicit_output

    return run_dir, stable_excel, compatibility_excel


def build_manifest(md_path, run_dir, report_context, chapter_records, focus_list, pending_updates):
    return {
...
def main():
    parser = argparse.ArgumentParser(description="开放式财务分析引擎")
    parser.add_argument("--md", required=True, help="输入 Markdown 路径")
    parser.add_argument("--output", help="输出目录或兼容模式下的 Excel 路径")
    parser.add_argument("--run-dir", help="显式指定运行目录")
    ...
    run_dir, stable_excel_path, compatibility_excel_path = resolve_output_paths(
        md_path,
        args.output,
        args.run_dir,
    )
    ...
    chapter_records = [
        build_chapter_record(index, chapter, report_context)
    ...
    if compatibility_excel_path:
        compatibility_excel_path.parent.mkdir(parents=True, exist_ok=True)
        shutil.copyfile(stable_excel_path, compatibility_excel_path)
