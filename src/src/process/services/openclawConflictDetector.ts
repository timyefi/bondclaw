 ...
 * as BondClaw Channels, and warns the user about the conflict.
 */
...
/**
 * Check if OpenClaw Lark channel conflicts with BondClaw credentials
 */
...
/**
 * Check if OpenClaw Telegram channel conflicts with BondClaw credentials
 */
...
export function getConflictResolutionSteps(platform: 'lark' | 'telegram'): string[] {
  ...
  return [
    `Detected conflict: OpenClaw ${platformName} channel is using the same credentials as BondClaw.`,
    ``,
    `This means messages are being handled by OpenClaw, not BondClaw Channels.`,
    `Switching agents in BondClaw will have no effect.`,
    ``,
