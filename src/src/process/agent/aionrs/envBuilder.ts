 ...
 * - Providers whose baseUrl already contains a version segment (e.g. `/v4`, `/v1`)
 *   also need api_path override to prevent aionrs from appending `/v1/chat/completions`.
...
function buildProjectConfig(model: TProviderWithModel, provider: AionrsProvider): string {
  ...
  // Custom providers with non-v1 versioned baseUrl (e.g. /v4, /v2, /v3) need api_path override.
  // Providers ending with /v1 are handled by stripTrailingV1() in resolveOpenAIBaseUrl flow,
  // so we only override api_path for /v2 and above to avoid double-appending.
  if (model.platform === 'custom') {
    const baseUrl = model.baseUrl || '';
    // Match version segments /v2, /v3, /v4, etc. (NOT /v1 which stripTrailingV1 handles)
    if (/\/v([2-9]\d*)(?:\/?$|\/)/.test(baseUrl)) {
      overrides.push('api_path = "/chat/completions"');
    }
  }
