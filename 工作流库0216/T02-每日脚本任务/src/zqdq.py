...
current_date1 = current_date + timedelta(days=30)
...
with sql_engine.begin() as connection:
    ...
    # dates0=pd.read_sql(query0,con=connection)
dates['dt']=pd.to_datetime(dates['dt'])
# dates0['dt']=pd.to_datetime(dates0['dt'])
print(f"{dt0},{dt1}")
date_set = set(dates['dt'].dt.date)

# for dt in dates0['dt']:
dt = current_date.date()
while dt <= current_date1.date():
    print(dt)
