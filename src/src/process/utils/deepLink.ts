...
export const PROTOCOL_SCHEME = 'bondclaw';
export const LEGACY_PROTOCOL_SCHEME = 'aionui';

/**
 * Parse a BondClaw deep link into action and params.
 * Supports two formats:
 *   1. bondclaw://add-provider?baseUrl=xxx&apiKey=xxx
 *   2. bondclaw://provider/add?v=1&data=<base64 JSON>  (one-api / new-api style)
 * Legacy `aionui://` links remain accepted for compatibility.
 */
export const parseDeepLinkUrl = (url: string): { action: string; params: Record<string, string> } | null => {
  try {
    const parsed = new URL(url);
    if (parsed.protocol !== `${PROTOCOL_SCHEME}:` && parsed.protocol !== `${LEGACY_PROTOCOL_SCHEME}:`) return null;
...
let pendingDeepLinkUrl: string | null =
  process.argv.find(
    (arg) => arg.startsWith(`${PROTOCOL_SCHEME}://`) || arg.startsWith(`${LEGACY_PROTOCOL_SCHEME}://`)
  ) || null;
