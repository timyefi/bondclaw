...
const initBuiltinAssistantRules = async (): Promise<void> => {
  ...
  for (const preset of ASSISTANT_PRESETS) {
    ...
    const hasTemplatePath = !!preset.templatePath;
    if (hasRuleFiles) {
    ...
    } else if (hasTemplatePath) {
      // Template-based presets: read prompt_template from JSON and write as rule file
      try {
        const templateFullPath = path.join(getPlatformServices().paths.getAppPath()!, preset.templatePath);
        if (existsSync(templateFullPath)) {
          const raw = await fs.readFile(templateFullPath, 'utf-8');
          const template = JSON.parse(raw);
          const promptTemplate = template.prompt_template || '';
          if (promptTemplate) {
            const targetPath = path.join(assistantsDir, `${assistantId}.en-US.md`);
            await fs.writeFile(targetPath, promptTemplate, 'utf-8');
          }
        }
      } catch (error) {
        console.warn(`[AionUi] Failed to write template rule for ${assistantId}:`, error);
      }
    } else {
...
const getBuiltinAssistants = (): AcpBackendConfig[] => {
  ...
  for (const preset of ASSISTANT_PRESETS) {
    ...
    const enabledByDefault =
      ...
      preset.id === 'social-job-publisher' ||
      preset.id === 'cowork' ||
      preset.id === 'openclaw-setup' ||
      preset.id === 'star-office-helper' ||
      preset.id === 'moltbook' ||
      preset.id === 'beautiful-mermaid' ||
      // First assistant in each role category is enabled by default
      preset.id === 'macro-daily-brief' ||
      preset.id === 'rates-curve-attribution' ||
      preset.id === 'credit-issuer-deep-dive' ||
      preset.id === 'convertibles-double-low-screening' ||
      preset.id === 'multi-asset-equity-bond-valuation' ||
      preset.id === 'fund-manager-morning-meeting-pack' ||
      preset.id === 'trader-inventory-scan';
...
const initStorage = async () => {
  ...
  try {
    ...
    // 5.2.4 检查是否需要迁移：为新添加的助手强制启用
    // Check if migration needed: force-enable newly added assistants for existing users
    const NEW_ASSISTANTS_MIGRATION_KEY = 'migration.newAssistantsEnabled_v2';
    const newAssistantsMigrationDone = await configFile.get(NEW_ASSISTANTS_MIGRATION_KEY).catch(() => false);
    const needsNewAssistantsMigration = !newAssistantsMigrationDone;
    const NEW_ASSISTANT_IDS = [
      'builtin-financial-analysis',
      'builtin-dashboard-creator',
      'builtin-financial-model-creator',
      'builtin-game-3d',
      'builtin-ui-ux-pro-max',
      'builtin-planning-with-files',
      'builtin-human-3-coach',
      'builtin-social-job-publisher',
      'builtin-pitch-deck-creator',
      // First assistant in each role category
      'builtin-macro-daily-brief',
      'builtin-rates-curve-attribution',
      'builtin-credit-issuer-deep-dive',
      'builtin-convertibles-double-low-screening',
      'builtin-multi-asset-equity-bond-valuation',
      'builtin-fund-manager-morning-meeting-pack',
      'builtin-trader-inventory-scan',
    ];
    ...
    for (const builtin of builtinAssistants) {
      ...
      if (index >= 0) {
        ...
        // 新添加的助手强制启用（迁移时）
        // Force-enable newly added assistants during migration
        const needsNewAssistantEnable =
          needsNewAssistantsMigration && NEW_ASSISTANT_IDS.includes(existing.id) && existing.enabled === false;
        // 迁移时强制使用默认值，否则保留用户设置
        // Force default value during migration, otherwise preserve user setting
        const resolvedEnabled = needsNewAssistantEnable ? true : needsEnabledFix ? builtin.enabled : existing.enabled;
    ...
    if (needsNewAssistantsMigration) {
      await configFile.set(NEW_ASSISTANTS_MIGRATION_KEY, true);
    }

    // 5.2.5 Clean up deprecated story-roleplay assistant
    const STORY_ROLEPLAY_CLEANUP_KEY = 'migration.storyRoleplayCleaned_v1';
    const storyRoleplayCleanupDone = await configFile.get(STORY_ROLEPLAY_CLEANUP_KEY).catch(() => false);
    if (!storyRoleplayCleanupDone) {
      // Remove story-roleplay from customAgents list
      const filtered = updatedAgents.filter((a: AcpBackendConfig) => a.id !== 'builtin-story-roleplay');
      if (filtered.length !== updatedAgents.length) {
        updatedAgents.length = 0;
        updatedAgents.push(...filtered);
        hasChanges = true;
      }
      // Remove cached rule/skill files
      const assistantsDir = getAssistantsDir();
      try {
        for (const file of readdirSync(assistantsDir)) {
          if (/^builtin-story-roleplay(-skills)?\..*\.md$/.test(file)) {
      ...
      await configFile.set(STORY_ROLEPLAY_CLEANUP_KEY, true);
    }
