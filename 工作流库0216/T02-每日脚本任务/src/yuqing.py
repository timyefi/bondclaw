...
def update_database(list_keywords,TypeInfo,happenDate,ContentInfo,ai_sum,cityFullNames,industryFullNames,id):
  sql = """
  INSERT ignore INTO 舆情监控 (list_keywords,TypeInfo,happenDate,ContentInfo,ai_sum,cityFullNames,industryFullNames,id)
  VALUES (:list_keywords, :TypeInfo, :happenDate, :ContentInfo, :ai_sum,:cityFullNames,:industryFullNames,:id)
  """
  # 构建参数字典
  params = {
    ...
    "ai_sum": ai_sum,
    "cityFullNames":cityFullNames,
    "industryFullNames":industryFullNames,
    "id":id
  }
  with sql_engine.begin() as connection:
    connection.execute(text(sql), params)
...
def pro_id(id,headers=headers_yy):
  ...
  cityFullNames0=r['result']['cityFullNames']
  cityFullNames=''
  if cityFullNames0==None:
    cityFullNames=''
  else:
    cityFullNames=''
    for i in range(len(cityFullNames0)):
      if cityFullNames0[i]==None:
        cityFullNames+=','
      else:
        cityFullNames+=cityFullNames0[i]+','
  
  industryFullNames0=r['result']['industryFullNames']
  if industryFullNames0==None:
    industryFullNames=''
  else:
    industryFullNames=''
    for i in range(len(industryFullNames0)):
      if industryFullNames0[i]==None:
        industryFullNames+=','
      else:
        industryFullNames+=industryFullNames0[i]+','
  ...
  return list_keywords,TypeInfo,happenDate,ContentInfo,ai_sum,cityFullNames,industryFullNames


def pro_yyinfo(dt_start,dt_end,headers=headers_yy):
  SkipCount=0
  # if dt_start=="2024-04-29":
  #   SkipCount=60
  # else:
  #   SkipCount=0
    ...
    while is_end==0:
      ...
      if totalCount>0:
        for item in r['result']['items']:
          ...
          list_keywords, TypeInfo, happenDate, ContentInfo, ai_sum,cityFullNames,industryFullNames = results
          print(ai_sum)
          update_database(list_keywords,TypeInfo,happenDate,ContentInfo,ai_sum,cityFullNames,industryFullNames,id)
      else:
        is_end=1
      if SkipCount+30>totalCount:
        ...
        SkipCount+=30
sql="""
select distinct id from yq.舆情监控
"""
with sql_engine.begin() as connection:
  df_id=pd.read_sql(sql,connection)
idlist=df_id['id'].tolist()
...
pro_yyinfo(formatted_date1,formatted_date)