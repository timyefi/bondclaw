...
| `DingTalkAdapter.ts` | Message format conversion (DingTalk â†?Unified)              |
...
```
PROCESSING (1) â†?INPUTING (2) â†?FINISHED (3)
                                  FAILED (5)
```
...
4. PUT /v1.0/card/streaming            # Stream content (repeat)
   ...
   - isFull: true                      # Full replacement (BondClaw sends complete content each time)
...
- **`isFull: true`** is always used because BondClaw sends the complete message content on each edit (not incremental deltas)

## Message Flow

```
DingTalk User â†?Stream WebSocket â†?DingTalkPlugin.handleRobotMessage()
  â†?ActionExecutor.handleIncomingMessage()
    â†?ChannelMessageService.sendMessage()
      â†?Agent (Gemini/ACP/Codex)
        â†?ChannelEventBus 'finish' event â†?Promise resolves
      â†?ActionExecutor sends final editMessage with replyMarkup
        â†?DingTalkPlugin.editMessage(isFinal=true)
          â†?streamAICard(isFinalize=true) + finishAICard()
```
