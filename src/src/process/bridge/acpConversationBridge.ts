...
export function initAcpConversationBridge(workerTaskManager: IWorkerTaskManager): void {
  ...
  ipcBridge.acpConversation.getAvailableAgents.provider(() => {
    try {
      ...
      // Ensure Claude is always the first agent in the list
      const claudeIdx = enriched.findIndex((a) => a.backend === 'claude');
      if (claudeIdx > 0) {
        const [claude] = enriched.splice(claudeIdx, 1);
        enriched.unshift(claude);
      }
