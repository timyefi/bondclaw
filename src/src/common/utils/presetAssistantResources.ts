...
export async function loadPresetAssistantResources(
  ...
  if (customAgentId.startsWith('builtin-')) {
    ...
    if (preset) {
      // Template-based presets: read prompt_template from JSON file
      if (!rules && preset.templatePath) {
        try {
          const templateContent = await deps.readBuiltinRule({ fileName: preset.templatePath });
          if (templateContent) {
            const template = JSON.parse(templateContent);
            rules = template.prompt_template || '';
          }
        } catch (error) {
          deps.warn(`[presetAssistantResources] Failed to load template for ${customAgentId}`, error);
        }
      }
