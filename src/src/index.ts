...
const isE2ETestMode = process.env.BONDCLAW_E2E_TEST === '1' || process.env.AIONUI_E2E_TEST === '1';
const skipSingleInstanceLock =
  isE2ETestMode || process.env.BONDCLAW_MULTI_INSTANCE === '1' || process.env.AIONUI_MULTI_INSTANCE === '1';
const deepLinkFromArgv = process.argv.find((arg) => arg.startsWith(`${PROTOCOL_SCHEME}://`));
const gotTheLock = skipSingleInstanceLock ? true : app.requestSingleInstanceLock({ deepLinkUrl: deepLinkFromArgv });
if (!gotTheLock) {
  console.warn('[BondClaw] Another instance is already running; current process will exit.');
  app.quit();
} else {
  app.on('second-instance', (_event, argv, _workingDirectory, additionalData) => {
    ...
    if (app.isReady()) {
      showOrCreateMainWindow({
        mainWindow,
        createWindow: () => {
          console.log('[BondClaw] second-instance received with no active main window, recreating main window');
          createWindow();
        },
      });
    }
...
const createWindow = ({ showOnReady = true }: { showOnReady?: boolean } = {}): void => {
  console.log('[BondClaw] Creating main window...');
  ...
  console.log(`[BondClaw] Main window created (id=${mainWindow.id})`);
  ...
  if (showOnReady) {
    const showWindow = () => {
      if (!mainWindow.isDestroyed() && !mainWindow.isVisible()) {
        console.log('[BondClaw] Showing main window');
        mainWindow.show();
        mainWindow.focus();
      }
    };
    mainWindow.once('ready-to-show', () => {
      console.log('[BondClaw] Window ready-to-show');
      showWindow();
    });
    // Belt-and-suspenders: also show on did-finish-load in case ready-to-show already fired
    mainWindow.webContents.once('did-finish-load', () => {
      console.log('[BondClaw] Renderer did-finish-load');
      showWindow();
    });
  ...
  const disableAutoUpdater =
    process.env.BONDCLAW_DISABLE_AUTO_UPDATE === '1' ||
  ...
  if (!disableAutoUpdater) {
    ...
    console.log('[BondClaw] Auto-updater disabled via env/CI guard');
  }
  ...
  if (!app.isPackaged && rendererUrl) {
    console.log(`[BondClaw] Loading renderer URL: ${rendererUrl}`);
    mainWindow.loadURL(rendererUrl).catch((error) => {
      console.error('[BondClaw] loadURL failed, falling back to file:', error.message || error);
      mainWindow.loadFile(fallbackFile).catch((e2) => {
        console.error('[BondClaw] loadFile fallback also failed:', e2.message || e2);
      });
    });
  } else {
    console.log(`[BondClaw] Loading renderer file: ${fallbackFile}`);
    mainWindow.loadFile(fallbackFile).catch((error) => {
      console.error('[BondClaw] loadFile failed:', error.message || error);
    });
  }

  mainWindow.webContents.on('did-fail-load', (_event, errorCode, errorDescription, validatedURL, isMainFrame) => {
    console.error('[BondClaw] did-fail-load:', { errorCode, errorDescription, validatedURL, isMainFrame });
  });

  mainWindow.webContents.on('render-process-gone', (_event, details) => {
    console.error('[BondClaw] render-process-gone:', details);
    ...
    if (!mainWindow.isDestroyed()) {
      console.log('[BondClaw] Attempting to recover from renderer crash by reloading...');

      if (!app.isPackaged && rendererUrl) {
        mainWindow.loadURL(rendererUrl).catch((error) => {
          console.error('[BondClaw] Recovery loadURL failed:', error.message || error);
        });
      } else {
        mainWindow.loadFile(fallbackFile).catch((error) => {
          console.error('[BondClaw] Recovery loadFile failed:', error.message || error);
        });
      }
    }
  });
  ...
  mainWindow.webContents.on('unresponsive', () => {
    console.warn('[BondClaw] Renderer became unresponsive');
  });

  mainWindow.on('closed', () => {
    console.log('[BondClaw] Main window closed');
  });
...
const handleAppReady = async (): Promise<void> => {
  ...
  const mark = (label: string) => console.log(`[BondClaw:ready] ${label} +${Math.round(performance.now() - t0)}ms`);
  mark('start');
  ...
  try {
    ...
    console.error('[BondClaw] Failed to restore zoom factor:', error);
...
// Register bondclaw:// as the default protocol client
...
// macOS: handle bondclaw:// URLs via the open-url event
...
void app
  ...
  .catch((error) => {
    ...
    console.error('[BondClaw] App initialization failed:', error);
    app.quit();
  });
...
app.on('before-quit', async () => {
  console.log('[BondClaw] before-quit');
  setIsQuitting(true);
  isExplicitQuit = true;
  destroyTray();
  // 在应用退出前清理工作进程
...
app.on('quit', (_event, exitCode) => {
  console.log(`[BondClaw] quit (exitCode=${exitCode})`);
});
