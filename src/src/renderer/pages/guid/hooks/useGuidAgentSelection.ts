  ...
  useEffect(() => {
    ...
    const loadLastSelectedAgent = async () => {
      try {
        ...
        // No saved preference or saved agent no longer available.
        // Prefer Claude Code globally, then fall back to the first available backend.
        const preferredFallbackKey =
          getAvailableFallbackAgent() || (availableAgents.length > 0 ? getAgentKey(availableAgents[0]) : null);
        if (preferredFallbackKey) {
          _setSelectedAgentKey(preferredFallbackKey);
        }
      } catch (error) {
