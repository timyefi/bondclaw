...
now_dts = pd.read_sql("SELECT DT FROM `stock`.`indexcloseprice` WHERE `TRADE_CODE` = '881001.WI' ORDER BY `DT` DESC limit 2", conn)['DT'].apply(lambda x:x.strftime('%F')).to_list()
for now_dt in now_dts[:1]:
    ...
    for marketinfo_name in marketinfo_list:
        ...
        col = ['ths_bond_balance_bond']
        perform_updates(true_trade_codes, col, table_name, prame_dic, duplicate_dic, now_dt, now_dts[now_dts.index(now_dt)+1])
        if marketinfo_name not in ['marketinfo_equity', 'marketinfo_rate']:
            del_cols = ['ths_bond_balance_bond', 'ths_remain_duration_y_bond', 'ths_repay_years_cb_bond', 'ths_repay_years_cb_bond_exercise']
        else:
            del_cols = ['ths_bond_balance_bond']
        for i in del_cols:
            del true_trade_codes[i]

        true_trade_codes_modify = query_trade_codes(conn, table_name, now_dt)
        ...
        col = 'all'
            # col = ['ths_bond_balance_bond', 'ths_cb_market_implicit_rating_bond', 'ths_evaluate_yield_cb_bond_exercise', 'ths_repay_years_cb_bond_exercise']
            # for i in col:
            #     if i not in prame_dic:
            #         raise KeyError(f"Key not found: {i}")

            # null_trade_codes = get_null_trade_codes(conn, table_name)
            # true_trade_codes = fetch_trade_codes_within_date_range(conn, table_name, now_dt, date_condition_type='exclude')
            # true_trade_codes = {
            #             'ths_evaluate_yield_cb_bond':true_trade_codes['ths_evaluate_yield_cb_bond'],
            #             'ths_repay_years_cb_bond_exercise':true_trade_codes['ths_repay_years_cb_bond_exercise'],
            #             'ths_subject_latest_rating_yy_bond':true_trade_codes['ths_subject_latest_rating_yy_bond'],
            # }
            # 执行更新逻辑
        if marketinfo_name not in ['marketinfo_equity', 'marketinfo_rate']:
            evaluate_cols = ['ths_evaluate_interest_durcb_bond_exercise', 'ths_evaluate_spread_durcb_bond_exercise', 'ths_evaluate_modified_dur_cb_bond_exercise']
            true_trade_codes_with_evaluate, true_trade_codes_without_evaluate = split_dict(true_trade_codes, evaluate_cols)
            result = perform_updates(true_trade_codes_without_evaluate, col, table_name, prame_dic, duplicate_dic, now_dt)

            # 先对有中债估值的有效trade_code取ths_evaluate_modified_dur_cb_bond_exercise
            # 后对有中债估值的有效且上一步没有得到值得trade_code 去取其他数 
            first_evaluate_cols = ['ths_evaluate_modified_dur_cb_bond_exercise']
            true_trade_codes_with_evaluate_before, true_trade_codes_with_evaluate_after = split_dict(true_trade_codes_with_evaluate, first_evaluate_cols)
            # 对字典中每个键对应的列表执行交集操作
            true_trade_codes_modify_exercise = query_trade_codes(conn, table_name, now_dt, col='ths_evaluate_yield_cb_bond_exercise')
            # print('true_trade_codes_with_evaluate_before', true_trade_codes_with_evaluate_before, true_trade_codes_modify_exercise)
            for key in true_trade_codes_with_evaluate_before:
                true_trade_codes_with_evaluate_before[key] = true_trade_codes_modify_exercise
            perform_updates(true_trade_codes_with_evaluate_before, col, table_name, prame_dic, duplicate_dic, now_dt)

            true_trade_codes_modify_exercise_before = query_trade_codes(conn, table_name, now_dt, col=first_evaluate_cols[0])
            # 对字典中每个键对应的列表执行差集操作
            evaluate_after_trade_codes =  list(set(true_trade_codes_modify_exercise) - set(true_trade_codes_modify_exercise_before))
            for key in true_trade_codes_with_evaluate_after:
                true_trade_codes_with_evaluate_after[key] = evaluate_after_trade_codes
            perform_updates(true_trade_codes_with_evaluate_after, col, table_name, prame_dic, duplicate_dic, now_dt)
        else:
            evaluate_cols = []
            true_trade_codes_with_evaluate, true_trade_codes_without_evaluate = split_dict(true_trade_codes, evaluate_cols)
            result = perform_updates(true_trade_codes_without_evaluate, col, table_name, prame_dic, duplicate_dic, now_dt)
