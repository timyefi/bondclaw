...
import type { AssistantPresetCategory } from '@/common/config/presets/assistantPresets';
...
import { Plus, Robot, Right } from '@icon-park/react';
...
const AssistantSelectionArea: React.FC<AssistantSelectionAreaProps> = ({
  ...
  const enabledPresets = useMemo(
    () => customAgents.filter((a) => a.isPreset && a.enabled !== false),
    [customAgents]
  );

  const categorizedAssistants = useMemo(() => {
    const categoryMap = new Map<string, AcpBackendConfig[]>();
    for (const agent of enabledPresets) {
      const cat = (agent as { category?: string }).category || 'general';
      const list = categoryMap.get(cat) || [];
      list.push(agent);
      categoryMap.set(cat, list);
    }
    return ASSISTANT_CATEGORIES.map((catDef) => ({
      id: catDef.id,
      name: catDef.nameI18n[localeKey] || catDef.nameI18n['en-US'] || catDef.id,
      assistants: categoryMap.get(catDef.id) || [],
    })).filter((c) => c.assistants.length > 0);
  }, [enabledPresets, localeKey]);
  ...
  // Categorized Assistant List View
  const renderAssistantChip = (assistant: AcpBackendConfig) => {
  ...
  return (
    <div className='mt-12px w-full'>
      {categorizedAssistants.map((category) => {
        ...
        return (
          <div key={category.id} className='mb-4px'>
            <div
              className='flex items-center gap-6px cursor-pointer px-8px py-6px rd-8px hover:bg-fill-2 transition-colors select-none'
            ...
            >
              ...
              />
              <span className='text-12px text-t-secondary font-medium'>{category.name}</span>
              <span className='text-11px text-t-4'>({category.assistants.length})</span>
            </div>
            {isExpanded && (
              <div className='flex flex-wrap gap-8px justify-center mt-4px px-4px'>
                {category.assistants.map((assistant) => renderAssistantChip(assistant))}
              </div>
            )}
          </div>
        );
