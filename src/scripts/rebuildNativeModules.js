 ...
 * Note: does NOT add 'vx' prefix here 鈥?the caller's cmdPrefix (e.g. 'vx --with msvc')
...
function getModulesToRebuild(platform) {
  ...
  // macOS: Skip node-pty (cross-compilation from ARM64鈫抶64 fails, use @lydell/node-pty-* prebuilts)
  if (platform === 'win32' || platform === 'windows') {
...
function rebuildSingleModule(options) {
  ...
  if (mustUsePrebuild) {
    console.log(`     Linux cross-compilation detected (${normalizedBuildArch} 鈫?${targetArch})`);
    ...
    if (fs.existsSync(prebuildsDir)) {
      ...
      if (hasNodeFile) {
        console.log(`     鉁?Found existing prebuilds in ${prebuildsDir}, skipping rebuild`);
  ...
  if (!forceRebuild || mustUsePrebuild) {
    try {
      ...
      console.log(`     鉁?prebuild-install succeeded`);
      return true;
    } catch (error) {
      if (mustUsePrebuild) {
        // For Linux cross-compile, prebuild-install MUST succeed
        console.error(`     鉁?prebuild-install failed and cross-compilation from source not supported`);
  ...
  if (!canCrossCompileFromSource(normalizedBuildArch, targetArch, platform)) {
    console.error(
      `     鉁?Cross-compilation from ${normalizedBuildArch} to ${targetArch} not supported on ${platform}`
    );
    return false;
  }
  ...
  try {
    ...
    console.error(`鉂?Failed to rebuild ${moduleName}:`, error.message);
    return false;
  }
}
