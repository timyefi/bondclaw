...
import BondClawQuickSetup from '@/renderer/pages/bondclaw/components/BondClawQuickSetup';
...
const ModelModalContent: React.FC = () => {
  ...
  // Agent detection
  const { data: detectedAgents, mutate: mutateAgents } = useSWR(
    'acp.agents.quicksetup',
    async () => {
      try {
  ...
  return (
    <div className='flex flex-col bg-2 rd-16px px-16px md:px-24px lg:px-28px py-16px md:py-18px'>
      ...
      {/* Quick Setup Section */}
      <div className='flex-shrink-0 pb-14px mb-14px border-b border-[var(--color-border-2)]'>
        <div className='text-18px font-600 text-t-primary mb-12px'>快速配置</div>
        <BondClawQuickSetup compact onConfigured={() => void mutate()} />
      </div>

      {/* Agent Detection Section */}
      <div className='flex-shrink-0 pb-14px mb-14px border-b border-[var(--color-border-2)]'>
        <div
          className='flex items-center justify-between cursor-pointer py-6px'
          onClick={() => setAgentsExpanded(!agentsExpanded)}
        >
          <span className='text-18px font-600 text-t-primary'>Agent 管理</span>
          <span className='text-12px text-t-secondary'>{agentsExpanded ? '收起' : '展开'}</span>
        </div>

        {agentsExpanded && (
          <div className='mt-12px space-y-12px'>
            {/* Claude Code */}
            <div className='flex items-center justify-between px-14px py-12px rd-12px bg-fill-1'>
              <div className='flex items-center gap-12px min-w-0'>
                <div className='w-36px h-36px rd-10px bg-[rgba(var(--orange-6),0.12)] flex items-center justify-center shrink-0'>
                  <span className='text-18px font-700 text-[rgb(var(--orange-6))]'>C</span>
                </div>
            ...
            {detectedAgents
              ?.filter((a: Record<string, any>) => a.backend !== 'claude')
              .map((agent: Record<string, any>) => (
                <div
                  key={agent.backend}
                  className='flex items-center justify-between px-14px py-10px rd-10px hover:bg-fill-1 transition-colors'
                >
                  <div className='flex items-center gap-10px min-w-0'>
                    <div className='text-13px font-500 text-t-primary'>{agent.name || agent.backend}</div>
                  </div>
                  <Tag color={agent.available !== false ? 'green' : 'gray'} size='small'>
                    {agent.available !== false ? '已就绪' : '不可用'}
                  </Tag>
                </div>
              ))}

            <div className='text-12px text-t-secondary'>
              Agent 在首次使用时会自动连接。如需管理更多 Agent，请前往高级配置。
            </div>
          </div>
        )}
      </div>
      ...
      <div className='flex-shrink-0'>
        <div
          className='flex items-center justify-between cursor-pointer py-10px px-12px rd-8px hover:bg-fill-2 transition-colors'
          onClick={() => setAdvancedExpanded(!advancedExpanded)}
        >
          <span className='text-14px font-500 text-t-secondary'>高级配置（{providerCount} 个服务商）</span>
          <span className='text-12px text-t-secondary'>{advancedExpanded ? '收起' : '展开'}</span>
        </div>
      </div>
        ...
        <>
          ...
          <div className='flex-shrink-0 border-b border-[var(--color-border-2)] pb-12px mb-14px flex flex-col gap-10px'>
            <div className='flex items-center justify-between gap-8px flex-wrap'>
              <div className='text-16px font-600 text-t-primary leading-28px'>{t('settings.model')}</div>
