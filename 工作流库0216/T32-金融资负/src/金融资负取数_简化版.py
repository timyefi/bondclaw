...
def update_financial_data(code, data_type, target_date):
    ...
    try:
        ...
        print(f"正在获取数据: {code}, 日期: {target_date_str}")
    ...
    try:
        with sql_engine.begin() as connection:
            ...
            if result.iloc[0]['count'] > 0:
                ...
                # 插入新记录
                insert_query = f"INSERT INTO 金融资负 (dt, {code}) VALUES (:dt, :value)"
                connection.execute(text(insert_query), {'dt': date, 'value': value})
            
            print(f"  数据更新成功: {code} = {value}")
            return True
                ...
                if not pd.isna(jsg):
                    ...
                    with sql_engine.begin() as connection:
                        try:
                            ...
                            print(f"  {code} 基金净申购数据更新成功: {jsg}")
                        except Exception as e:
                            print(f"  {code} 数据可能已存在: {e}")
                else:
                    print(f"  {code} 无有效数据")
            else:
                print(f"  Wind接口获取 {code} 数据失败: {wsd_data.ErrorCode}")
                
    except Exception as e:
        print(f"更新基金净申购数据时出错: {e}")

def main():
    """主函数"""
    print("=" * 50)
    print("金融资负数据取数程序（简化版）")
    print("=" * 50)
    ...
    print("\n2. 检查是否需要更新金融资负数据...")
    if is_update_period():
        print("  当前在更新期间（每月20-30号），开始更新金融资负数据...")
        ...
        print(f"  Wind数据更新完成，成功: {success_count}/{len(wind_codes)}")
        ...
        print(f"  同花顺数据更新完成，成功: {success_count}/{len(ths_codes)}")
