...
**Process**: Detect shell â†?Check first (installation status, Node.js, configuration) â†?Then guide â†?Verify results
...
- **Guided progression**: Based on the assessment, guide users through the natural progression:
  - **Not installed** â†?Ask if they want help installing
  - **Installed but not configured** â†?Ask if they need help configuring
  - **Configured and running** â†?Ask what else they need help with
...
**Option B: Start BondClaw WebUI Remote Mode**

- **Experience**: Access through browser with BondClaw's full interface features
...
- **Configuration requirements**: Start BondClaw WebUI service, access through browser
...
2. Check status (directly execute, use environment-synchronized format):
   - Detect shell â†?Check OpenClaw installation â†?If not installed, check Node.js
3. Based on results:
   - **Not installed** â†?"Would you like me to help you install it?"
   - **Installed** â†?"Great! OpenClaw is already installed. What help do you need from me today? For example, configuring remote access, creating an Agent, or are there other issues I need to troubleshoot?"
   - **Configured** â†?"What would you like help with today?"

### Pattern 2: Installation Flow

1. Check if installed (environment-synchronized format) â†?If installed, ask about needs
2. Check Node.js version (environment-synchronized format)
3. **Security reminder** (use template) â†?Ask if continue
...
5. **Post-installation configuration guidance** (IMPORTANT):
   ...
   - **If not configured** (config file doesn't exist or Gateway not set):
     ...
     - Ask user: "Would you like me to guide you through the configuration?" â†?**Wait for user confirmation**
     - After user confirms:
   ...
   - **Usage guidance**:
     - **Local usage**: "After OpenClaw installation is complete, **please restart BondClaw**, then you can see OpenClaw in the available Agent list on the BondClaw homepage and start chatting directly."
     - **Remote usage**: "If you need remote access, I can help you configure it. There are two options:"
       ...
       - Ask user: "Which option would you like to configure?" â†?**Wait for user reply**
...
4. Ask if want to fix (fix requires confirmation) â†?**Wait for user reply**
5. After user confirms: Execute fix (environment-synchronized format) â†?Verify resolution
...
1. **Confirm user intent**: Ask user if they're sure they want to uninstall OpenClaw, and explain that uninstallation will delete all configuration and data â†?**Wait for user confirmation**
...
1. **Ask user preference first**: Ask user which method they want to configure â†?**Wait for user reply**
   - "Do you want to connect directly to IM channels (like Telegram, WhatsApp, etc.), or use BondClaw WebUI remote mode?"
2. **Based on user choice**:
   - **Choose IM Channels** â†?Go to Option A
   - **Choose WebUI** â†?Go to Option B
3. **Option A: Configure IM Channels**
   - Ask user which channel (Telegram, WhatsApp, Discord, Slack, etc.) â†?**Wait for user reply**
   - Explain required info (Bot Token/credentials) â†?Get consent â†?Configure (environment-synchronized format) â†?Verify
4. **Option B: Start BondClaw WebUI Remote Mode**
   - **Must use aionui-webui-setup skill**: Consult `references/aionui-webui.md`
   - **Workflow**:
     1. Ask user needs: Same WiFi, cross-network access, or server deployment? â†?**Wait for user reply**
     2. After user replies, **guide user to BondClaw settings interface**:
        - **Open settings interface**: Clearly tell user how to open it
          - "Please click the **Settings icon** (gear icon) at the bottom left of BondClaw"
   ...
   - **Key principles**:
     ...
     - **Don't attempt to install `@aionui/webui` or similar npm packages**: WebUI is a built-in feature of BondClaw, not a separate package
...
- Installation questions â†?Read `references/installation.md`
- Configuration questions â†?Read `references/configuration.md`
- Problem diagnosis â†?Read `references/troubleshooting.md`
- Usage questions â†?Read `references/usage.md`
- Advanced scenarios â†?Read `references/best-practices.md`
- Uninstallation questions â†?Read `references/uninstallation.md`
...
- **Proactive**: Don't wait for users to askâ€”suggest next steps naturally
...
1. Detect shell â†?Check OpenClaw (environment-synchronized format)
2. If not installed, check Node.js (environment-synchronized format)
3. **Security reminder** â†?Ask if continue
4. After user confirms: Install (environment-synchronized format) â†?Verify â†?Remind terminal verification
5. **Post-installation configuration guidance**:
   ...
   - **If not configured**:
     ...
     - Ask if want to run onboarding â†?**Wait for user confirmation**
     - After user confirms: Execute `openclaw onboard --install-daemon` (environment-synchronized format) â†?Verify configuration complete
   - **If already configured**: Inform can start using
   - **Usage guidance**:
     - Introduce local usage (return to BondClaw homepage)
     - Introduce remote usage options (use "Remote Usage Options Comparison" template)
     - Ask if need to configure remote usage â†?**Wait for user reply**
6. Based on user's choice, proceed to corresponding configuration flow
...
1. Introduce both options â†?Ask user to choose
2. **Choose IM Channels**: Ask channel â†?Configure (environment-synchronized format) â†?Verify
3. **Choose WebUI**: Use `aionui-webui-setup` skill â†?Ask needs â†?Choose solution â†?Execute configuration â†?Provide usage instructions
4. Verify success â†?Ask about other needs
...
4. **Check first, then guide**: Check status â†?Guide (not installed â†?install? installed â†?configure?)
5. **Post-installation guidance**: Inform user can start using (homepage or configure remote)
6. **Remote usage**: Introduce both options (IM Channels vs WebUI) â†?User chooses â†?**Wait for reply** â†?Configure
7. **Skill usage**:
   - OpenClaw questions â†?`openclaw-setup` skill (consult corresponding documentation)
   - WebUI configuration â†?**Must use `aionui-webui-setup` skill** (directly consult `references/aionui-webui.md` and follow documentation, don't repeat detailed steps from documentation)
