...
import { ASSISTANT_CATEGORIES, getCategoryName } from '@/common/config/presets/assistantPresets';
import { useLayoutContext } from '@/renderer/hooks/context/LayoutContext';
import type { AssistantListItem } from './types';
import AssistantAvatar from './AssistantAvatar';
import { Button, Input, Switch, Tabs, Tag } from '@arco-design/web-react';
import { Plus, Search, SettingOne, CloseSmall, Right } from '@icon-park/react';
...
const AssistantListPanel: React.FC<AssistantListPanelProps> = ({
  ...
  const filteredAssistants = useMemo(() => {
    let result = filterAssistants(assistants, searchQuery, activeFilter, localeKey);
    // Apply category filter for builtin tab
    if (activeFilter === 'builtin' && categoryFilter !== 'all') {
      result = result.filter(
        (a) => (a as { category?: string }).category === categoryFilter || (!a.category && categoryFilter === 'general')
      );
    }
    return result;
  }, [activeFilter, assistants, localeKey, searchQuery, categoryFilter]);
    ...
    <div className='py-2'>
      <div className={`bg-fill-2 rounded-24px ${isMobile ? 'p-16px' : 'p-20px'}`}>
        <div className='flex flex-col gap-14px mb-20px'>
          ...
          {/* Category filter for builtin tab */}
          {activeFilter === 'builtin' && (
            <div className='flex flex-wrap gap-8px mt-4px'>
              <Tag
                ...
                className='!rounded-16px !text-12px'
              >
                {t('settings.assistantFilterAll', { defaultValue: 'All' })}
              </Tag>
              {ASSISTANT_CATEGORIES.map((cat) => {
                const count = assistants.filter(
                  (a) =>
                    getAssistantSource(a) !== 'custom' &&
                    ((a as { category?: string }).category === cat.id ||
                      (!a.category && cat.id === 'general'))
                ).length;
                ...
                return (
                  <Tag
                    ...
                    className='!rounded-16px !text-12px'
                  >
                    {getCategoryName(cat.id, localeKey)} ({count})
                  </Tag>
                );
              })}
            </div>
        ...
        {filteredAssistants.length > 0 ? (
          <div className='space-y-16px'>
            {/* In builtin tab with "all" category, group by category */}
            {activeFilter === 'builtin' && categoryFilter === 'all' ? (
              <>
                {ASSISTANT_CATEGORIES.map((catDef) => {
                  const catAssistants = filteredAssistants.filter(
                    (a) =>
                      (a as { category?: string }).category === catDef.id ||
                      (!(a as { category?: string }).category && catDef.id === 'general')
                  );
                  if (catAssistants.length === 0) return null;
                  const isExpanded = expandedCategories[catDef.id] ?? catDef.id === 'general';
                  const { enabledAssistants: catEnabled, disabledAssistants: catDisabled } =
                    groupAssistantsByEnabled(catAssistants);
                  return (
                    <div key={catDef.id}>
                      <div
                        className='flex items-center gap-6px cursor-pointer px-4px py-6px hover:bg-fill-1 transition-colors select-none rd-8px'
                        onClick={() =>
                          setExpandedCategories((prev) => ({ ...prev, [catDef.id]: !prev[catDef.id] }))
                        }
                      >
                        <Right
                          theme='filled'
                          size={12}
                          fill='currentColor'
                          className={`text-t-3 transition-transform duration-200 ${isExpanded ? 'rotate-90' : ''}`}
                        />
                        <span className='text-13px font-medium text-t-secondary'>
                          {getCategoryName(catDef.id, localeKey)}
                        </span>
                        <span className='text-11px text-t-4'>({catAssistants.length})</span>
                      </div>
                      {isExpanded && (
                        <div className='space-y-12px mt-8px'>
                          {catEnabled.length > 0 && renderSection(t('settings.assistantSectionEnabled', { defaultValue: 'Enabled' }), catEnabled)}
                          {catDisabled.length > 0 && renderSection(t('settings.assistantSectionDisabled', { defaultValue: 'Disabled' }), catDisabled)}
                        </div>
                      )}
                    </div>
                  );
